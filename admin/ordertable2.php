...
<tr>
<th>Order Id</th>
<th>Chicken Biryani Quantity</th>
<th>Goan Thali Quantity</th>
<th>Leaf Thali Quantity</th>
...
$y=$_POST['todate'];
echo "$x to $y";
$qry="SELECT * from dinnerorders WHERE date BETWEEN '$x' AND '$y'";
$res=mysql_query($qry);
$total=0;
	while($menu=mysql_fetch_assoc($res)) {
		echo "<tr>";
		echo "<td><center>".$menu["id"]."</td>";
		echo "<td><center>".$menu["chicken_biryani"]."</td>";
		echo "<td><center>".$menu["goan_thali"]."</td>";
		echo "<td><center>".$menu["leaf_thali"]."</td>";
...
$x=$_POST['id'];
echo $x;
$sql="delete from dinnerorders where id='$x'";
mysql_query($sql);
